...
func LocalOrderBook(ctx context.Context, ws *bnws.WsService, cps []string) {
	callBack := bnws.NewCallBack(func(msg *bnws.UpdateMsg) {
		queue.Put(msg)
	...
	channel := bnws.ChannelSpotOrderBookUpdate
	...
	for {
		...
		default:
			...
			var depthMsg bnws.SpotUpdateDepthMsg
			if err := json.Unmarshal(msg.(*bnws.UpdateMsg).Result, &depthMsg); err != nil {
				log.Printf("order Unmarshal err:%s", err.Error())
			}
			if v, ok := spotMsg.Load(depthMsg.CurrencyPair); ok {
				if v.(bnws.SpotUpdateDepthMsg).LastId+1 != depthMsg.FirstId {
					log.Printf("%s order book msg id discontinuous, old id %d-%d, new msg id %d-%d", depthMsg.CurrencyPair,
						v.(bnws.SpotUpdateDepthMsg).FirstId, v.(bnws.SpotUpdateDepthMsg).LastId, depthMsg.FirstId, depthMsg.LastId)
				}
			}
...
func updateLocalOrderBook(msg bnws.SpotUpdateDepthMsg) error {
...
func reGetBaseDepth(nowID int64, msg bnws.SpotUpdateDepthMsg) (err error) {
	var depth *OrderBook
...
func updateOrderBook(orderBook *OrderBook, update bnws.SpotUpdateDepthMsg) error {
